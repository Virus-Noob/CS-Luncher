...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("CS-Lunch was updated to the latest version")
        mesgdcrt.GeneralMessage("Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update CS-Lunch.")
        mesgdcrt.WarningMessage("Grab The Latest one From https://github.com/Virus-Noob/CS-Lunch.git")
...
def do_git_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("CS-Luncher was updated to the latest version")
        mesgdcrt.GeneralMessage("Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update CS-Luncher.")
        mesgdcrt.WarningMessage("Make Sure To Install 'git' ")
        mesgdcrt.GeneralMessage("Then run command:")
        print("git checkout . && git pull https://github.com/Virus-Noob/CS-Luncher.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get("https://raw.githubusercontent.com/Virus-Noob/CS-Luncher/main/.version").text.strip()
    if fver != __VERSION__:
        mesgdcrt.WarningMessage("An update is available")
        mesgdcrt.GeneralMessage("Starting update...")
        update()
    else:
        mesgdcrt.SuccessMessage("CS-Luncher is up-to-date")
        mesgdcrt.GeneralMessage("Starting CS-Luncher")

def notifyen():
    try:
        noti = requests.get("https://raw.githubusercontent.com/Virus-Noob/CS-Luncher/main/.notify").text.upper().strip()
        if len(noti) > 10:
...
def pretty_print(cc,target,success,failed):
    ...
    mesgdcrt.GeneralMessage("Please stay connected to the internet during lunching")
    ...
    mesgdcrt.SuccessMessage("CS-Luncher was created by Virus-Noob")
...
def workernode(mode,cc,target,count,delay,max_threads):
    ...
    clr()
    mesgdcrt.SectionMessage("Gearing up the Luncher - Please be patient")
    mesgdcrt.GeneralMessage("Please stay connected to the internet during lunching")
    mesgdcrt.GeneralMessage("Target        : " + cc + target)
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit={"sms":500,"call":20,"mail":200}
        cc,target="",""
        if mode in ["sms","call"]:
...
if sys.version_info[0]!=3:
    mesgdcrt.FailureMessage("CS-Luncher will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['SpeedX','t0xic0der','scpketer','Stefan','Virus-Noob']

ALL_COLORS = [Fore.GREEN, Fore.RED, Fore.YELLOW, Fore.BLUE, Fore.MAGENTA, Fore.CYAN, Fore.WHITE]
RESET_ALL = Style.RESET_ALL

description="""CS-Luncher - Your Friendly Spammer Application

CS-Luncher can be used for many purposes which incudes - 
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

CS-Luncher is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,epilog='Coded by Virus-Noob !!!')
parser.add_argument("-sms","--sms", action="store_true",help="start CS-Luncher with SMS Lunch mode")
parser.add_argument("-call","--call", action="store_true",help="start CS-Luncher with CALL Lunch mode")
parser.add_argument("-mail","--mail", action="store_true",help="start CS-Luncher with MAIL Lunch mode")
parser.add_argument("-u","--update", action="store_true",help="update CS-Luncher")
parser.add_argument("-c","--contributors", action="store_true",help="show current CS-Luncher contributors")
parser.add_argument("-v","--version", action="store_true",help="show current CS-Luncher version")
